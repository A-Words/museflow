...
stateDiagram-v2
    [*] --> queued: 校验和冻结成功
    queued --> running: Worker 领取且执行前检查通过
...
终态不可被重复或乱序回调覆盖。`queued` 取消与 Worker 领取用同一行锁/版本条件互斥。运行中取消不保证成功；Provider 不支持取消时返回 `CANCEL_NOT_SUPPORTED`，原任务继续。对结果未知的任务，界面显示“正在核对”，不显示可再次付费重试按钮。
...
| Worker 领取后崩溃，尚未调用 Provider | 通过持久化 attempt 与派发阶段判断；只有证明未派发或 Provider 支持相同 key 去重时才安全提交 |
...
| Worker 租约过期但旧进程仍在运行 | 使用递增租约令牌和任务版本拒绝旧进程提交；外部调用还需 Provider 幂等保障，租约本身不保证供应商只执行一次 |
