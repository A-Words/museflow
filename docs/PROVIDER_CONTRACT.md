...
- `query`/`cancel` 必须用该请求被接受时返回的厂商 `requestId` 寻址：`query` 返回 `completed`、`accepted`、`canceled` 前都要核对已接受请求及 `requestId`，`cancel` 返回 `canceled` 前也须核对；完成结果沿用提交时确认的参数（时长、格式、语言、音色）。查不到记录或 `requestId` 不匹配时返回 `unknown`（`query-unavailable`），不得伪造请求状态。
