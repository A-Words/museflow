...
> **本节已展开细化。** 完整的实现级契约（三类端口的操作签名、能力声明结构、配置版本与快照、规范化输入输出、错误分类与重试策略矩阵、两套可控配置与能力不匹配样例）见 **[Provider 内部契约](PROVIDER_CONTRACT.md)**。本节保留概要，两者冲突时以 `PROVIDER_CONTRACT.md` 为准。
