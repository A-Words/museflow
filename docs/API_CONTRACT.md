...
MF-04 已实现公共存活检查 `GET /api/v1/health`，返回 `{ "status": "ok", "service": "web" }`，schema 位于 `@museflow/contracts`。该接口不执行数据库查询，也不表示 Worker、数据库或 Provider 就绪，不返回配置值。下表其余业务路由仍是待实现契约。
