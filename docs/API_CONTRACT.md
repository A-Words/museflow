...
MF-03 已把本节细化为可执行契约：三类端口的输入输出与调用模式、能力声明、规范化错误与重试分级、配置版本与快照、固定原供应商规则，以及两套可控配置的 Mock 成功/失败/未知样例，见 [Provider 端口契约](PROVIDER_CONTRACT.md)。运行时 schema 位于 `shared/contracts/provider/`，Mock 适配器与注册表位于 `server/services/providers/`。本节保留目标与边界概述；真实适配器与联调仍由后续 Issue 完成。
